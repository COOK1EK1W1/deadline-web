...
> = {
  ...
  transformers?: Partial<
    Record<TransformerKey, (value: string) => T[TransformerKey]>
  >;
   ...
   */
  formatters?: Partial<
    Record<FormatterKey, (value: T[FormatterKey]) => string>
  >;
...
>({
  ...
  onChange,
  children,
}: Props<T, TransformerKey, FormatterKey>) {
  ...
  const setData = useCallback(
    (key: TransformerKey, value: string) => {
      ...
      const transformedValue =
        (transformers && transformers[key]?.(value)) ?? value;
  ...
  return (
    ...
    <FormContext.Provider
      value={{ getData: getData as any, setData: setData as any }}
    >
      <form
